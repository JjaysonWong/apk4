...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('apk4_app_list', function (Blueprint $table) {
            $table->increments('id');
            ...
            $table->string('keywords', 70)->default('')->comment('关键字');
            ...
            $table->string('tags', 500)->nullable()->comment('关联标签id');
            ...
            $table->tinyInteger('isshow')->default(2)->index('isshow')->comment('是否列表显示 1是 2否');
            $table->boolean('is_down')->nullable()->default(false)->comment('是否打包 ');
            $table->string('union_id', 50)->nullable()->unique('union_id')->comment('唯一字符串');
            $table->tinyInteger('source_id')->default(0)->comment('来源id');
            $table->string('source_url', 500)->default('')->comment('来源url');
    ...
    {
        Schema::dropIfExists('apk4_app_list');
    }
};
