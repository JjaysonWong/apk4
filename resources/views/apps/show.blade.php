    ...
    <div class="downloadPage">
        <div class="container">
            ...
            <div class="banner-download">
                <img class="appImage"
                    src="{{ Str::startsWith($app->icon, ['http://', 'https://']) ? $app->icon : config('app.img_db') . $app->icon }}"
                    alt="App Image" />
                <div class="detailWrap">
                    <div class="name">{{ $app->name }}</div>
                    <div class="detail">
                        <p>{{ $app->seo_description }}</p>
                    </div>
                </div>
                <div class="downloadButtonWrap">
                    ...
                    <div class="buttonWrap">
                        <div>
                            <a href="{{ $app->android_url ?? '#' }}" target="_blank"
                        ...
                        <div>
                            <a href="{{ $app->pc_url ?? '#' }}" target="_blank"
                                class="row d-flex justify-content-center align-items-center w-100 custom-link"
                                style="display: flex; align-items: center; text-decoration: none; width: 40%; height: 100%;">
                                <img src="{{ asset('images/download/xiazai.png') }}" alt="Download Icon"
                                    style="margin-right: 5px;" />
                ...
                <div class="content-left">
                    ...
                    <div class="app-info">
                        ...
                        <div class="update-info">
                            ...
                            <p class="appVersion">{!! $app->app_version !!} / {!! $app->uptime !!}</p>
                            <p class="appUpdateInfo">{{ __('categories.' . $app['category']) }}</p>
                        </div>
                        <div class="related-history-version">
                            ...
                            <div class="downloadList">
                                <div class="downloadWrap">
                                    <div class="titleWrap">
                                        <p>{!! $app->name !!} {!! $app->app_version !!}</p>
                                        <div class="dateMemory">
                                            <p>{!! $app->size !!}MB</p>
                                            <p>{{ date('Y-m-d', strtotime($app->uptime)) }}</p>
                                        </div>
                                    </div>
                    ...
                    <div class="similar-app-list">
                        <div class="titleWrap">
                            <h2>{{ __('auth.similar_apps') }}</h2>
                            <a href="{{ route('app.category', ['category' => $app->category]) }}" target="_blank"
                        ...
                        <div class="appList">
                            @foreach ($similarApps as $similarApp)
                                <div class="appWrap">
                                    <a href="{{ route('app.show', ['union_id' => $similarApp['union_id']]) }}">
                                        <img src="{{ Str::startsWith($similarApp['icon'], ['http://', 'https://']) ? $similarApp['icon'] : config('app.img_db') . $similarApp['icon'] }}"
                                            alt="Game Image" />
                                    </a>
                                    <p class="name">{{ $similarApp['name'] }}</p>
                                    <p class="dateUpdate">{{ date('Y-m-d', $similarApp['uptime']) }}更新</p>
                                    <div class="mobileStarWrap">
                                        <img src="{{ asset('images/download/star-fill.png') }}" alt="Star Fill" />
                                        <p>{{ $similarApp['game_score'] }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="appList">
                            @foreach ($recommendedApps as $recommendApp)
                                <div class="appWrap">
                                    ...
                                    <p class="name">{{ $recommendApp['name'] }}</p>
                                    <p class="dateUpdate">{{ date('Y-m-d', $recommendApp['uptime']) }}更新</p>
                                    <div class="mobileStarWrap">
                                        <img src="{{ asset('images/download/star-fill.png') }}" alt="Star Fill" />
                                        <p>{{ $recommendApp['game_score'] }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="hot-app">
                        ...
                        <div class="app-list">
                            @foreach ($hotApps as $hotApp)
                                <div class="appWrap">
                                    <img
                                        src="{{ Str::startsWith($hotApp['icon'], ['http://', 'https://']) ? $hotApp['icon'] : config('app.img_db') . $hotApp['icon'] }}" />
                                    <div class="appDetailWrap">
                                        <p class="appName">{{ $hotApp['name'] }}</p>
                                        <p class="appType">
                                            {{ __('categories.' . $hotApp['catalog']) }}
                                        </p>
                                        <p class="dateUpdate">{{ date('Y-m-d', $hotApp['uptime']) }}更新</p>
                                    </div>
                                    <a href="{{ route('app.show', ['union_id' => $hotApp['union_id']]) }}"
                    ...
                    <div class="hot-topic-section">
                        ...
                        <div class="hot-topic-listing">
                            ...
                            @foreach ($appCategories as $category)
                                <div onclick="document.querySelectorAll('.hot-topic-listing div').forEach(el => el.classList.remove('active')); this.classList.add('active'); window.location.href='{{ route('app.category', ['category' => $category['catalog']]) }}';"
                                    class="{{ $category['catalog'] == $app->category ? 'active' : '' }}"
